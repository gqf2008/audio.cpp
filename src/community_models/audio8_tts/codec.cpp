...
constexpr int64_t kCodecTransformerKVHeads = 8;
constexpr int64_t kCodecHeadDim = 64;
constexpr int64_t kCodecIntermediate = 3072;
constexpr int64_t kCodecPostIntermediateSize = 1216;
...
struct CodecTransformerWeights {
    ...
    int64_t kv_heads = kCodecTransformerKVHeads;
    int64_t intermediate_size = kCodecIntermediate;
};
...
std::vector<float> normalized_rows(const std::vector<float> & values, int64_t rows, int64_t cols) {
    if (static_cast<int64_t>(values.size()) != rows * cols) {
        throw std::runtime_error("Audio8 TTS normalized_rows shape mismatch");
    }
...
core::TensorValue slice_frames(core::ModuleBuildContext & ctx, const core::TensorValue & input, int64_t start, int64_t frames) {
    if (frames <= 0) {
        throw std::runtime_error("Audio8 TTS codec slice_frames requires positive frames");
    }
...
core::TensorValue zero_prefix_like(core::ModuleBuildContext & ctx, const core::TensorValue & input, int64_t frames) {
    if (frames <= 0) {
        throw std::runtime_error("Audio8 TTS zero_prefix_like requires positive frames");
    }
...
core::TensorValue zero_suffix_like(core::ModuleBuildContext & ctx, const core::TensorValue & input, int64_t frames) {
    if (frames <= 0) {
        throw std::runtime_error("Audio8 TTS zero_suffix_like requires positive frames");
    }
...
core::TensorValue causal_pad(core::ModuleBuildContext & ctx, const core::TensorValue & input, int64_t left_pad, int64_t right_pad) {
    if (left_pad < 0) {
        throw std::runtime_error("Audio8 TTS causal conv requires non-negative left padding");
    }
    if (right_pad < 0) {
        throw std::runtime_error("Audio8 TTS causal conv requires non-negative right padding");
    }
...
core::TensorValue build_mlp(
    ...
    auto up = modules::LinearModule({kCodecDim, intermediate_size, false, GGML_PREC_F32})
                  .build(ctx, input, weights.up_proj);
    auto hidden = modules::MulModule{}.build(ctx, gate, up);
    return modules::LinearModule({intermediate_size, kCodecDim, false, GGML_PREC_F32})
        .build(ctx, hidden, weights.down_proj);
}

core::TensorValue reshape_heads(core::ModuleBuildContext & ctx, const core::TensorValue & input, int64_t heads) {
    const auto contiguous = core::ensure_backend_addressable_layout(ctx, input);
    return core::reshape_tensor(
        ctx,
        contiguous,
        core::TensorShape::from_dims({input.shape.dims[0], input.shape.dims[1], heads, kCodecHeadDim}));
}
...
core::TensorValue build_transformer_layer(
    ...
    auto k = modules::LinearModule({kCodecDim, kv_heads * kCodecHeadDim, false, GGML_PREC_F32})
                 .build(ctx, normed, {weights.attention.k_weight, std::nullopt});
    auto v = modules::LinearModule({kCodecDim, kv_heads * kCodecHeadDim, false, GGML_PREC_F32})
                 .build(ctx, normed, {weights.attention.v_weight, std::nullopt});
    // modeling_arktts_codec.py:98-100: flash attn maps q head i to kv head i/(Hq/Hkv),
    // identical to the reference repeat_interleave expansion.
    q = modules::RoPEModule({kCodecHeadDim, GGML_ROPE_TYPE_NORMAL, kCodecRopeTheta}).build(ctx, reshape_heads(ctx, q, kCodecTransformerHeads), positions);
    k = modules::RoPEModule({kCodecHeadDim, GGML_ROPE_TYPE_NORMAL, kCodecRopeTheta}).build(ctx, reshape_heads(ctx, k, kv_heads), positions);
    v = reshape_heads(ctx, v, kv_heads);
    ...
    auto ff = build_mlp(ctx, ffn_in, weights.feed_forward, intermediate_size);
...
core::TensorValue build_window_transformer(
    ...
    for (const auto & layer : weights.layers) {
        x = build_transformer_layer(ctx, x, positions, mask, layer, weights.kv_heads, weights.intermediate_size);
    }
...
core::TensorValue build_encode_quantizer(
    ...
    trace_outputs.push_back({"audio8_tts.codec.after_downsample", x});
...
CodecTransformerWeights load_transformer(
    ...
    for (int64_t layer = 0; layer < layers; ++layer) {
        ...
        // arktts packs QKV as one [(n_head + 2*n_local_heads)*head_dim, dim]
        // matrix (modeling_arktts_codec.py:79); split rows into q/k/v at load time.
        const auto packed_qkv = source.require_tensor(
            layer_prefix + ".attention.wqkv.weight",
            storage_type,
            {(kCodecTransformerHeads + 2 * kv_heads) * kCodecHeadDim, kCodecDim});
        const size_t qkv_row_bytes = ggml_row_size(packed_qkv.type, kCodecDim);
        const size_t q_rows = static_cast<size_t>(kCodecTransformerHeads) * kCodecHeadDim;
        const size_t kv_rows = static_cast<size_t>(kv_heads) * kCodecHeadDim;
        const std::byte * qkv_data = packed_qkv.bytes.data();
        weights.attention.q_weight = store.make_tensor(
            core::TensorShape::from_dims({static_cast<int64_t>(q_rows), kCodecDim}),
            packed_qkv.type,
            qkv_data,
            qkv_row_bytes * q_rows);
        weights.attention.k_weight = store.make_tensor(
            core::TensorShape::from_dims({static_cast<int64_t>(kv_rows), kCodecDim}),
            packed_qkv.type,
            qkv_data + qkv_row_bytes * q_rows,
            qkv_row_bytes * kv_rows);
        weights.attention.v_weight = store.make_tensor(
            core::TensorShape::from_dims({static_cast<int64_t>(kv_rows), kCodecDim}),
            packed_qkv.type,
            qkv_data + qkv_row_bytes * (q_rows + kv_rows),
            qkv_row_bytes * kv_rows);
        ...
        weights.feed_forward.gate_proj.weight = store.load_tensor(source, layer_prefix + ".feed_forward.w1.weight", storage_type, {intermediate_size, kCodecDim});
        weights.feed_forward.down_proj.weight = store.load_tensor(source, layer_prefix + ".feed_forward.w2.weight", storage_type, {kCodecDim, intermediate_size});
        weights.feed_forward.up_proj.weight = store.load_tensor(source, layer_prefix + ".feed_forward.w3.weight", storage_type, {intermediate_size, kCodecDim});
...
std::shared_ptr<FishCodecWeights> load_weights(
    ...
    weights->store = std::make_shared<core::BackendWeightStore>(backend, backend_type, "Audio8 TTS codec", weight_context_bytes);
    ...
    for (int64_t block_index = 0; block_index < 4; ++block_index) {
        ...
        encoder_channels *= 2;
        if (block_index == 3) {
            block.transformer = load_transformer(store, source, prefix + ".5", matmul_storage_type, 4, kCodecTransformerHeads, kCodecIntermediate);
        }
    ...
    weights->pre_module = load_transformer(store, source, "quantizer.pre_module", matmul_storage_type, kCodecTransformerLayers, kCodecTransformerHeads, kCodecIntermediate);
    ...
    weights->post_module = load_transformer(store, source, "quantizer.post_module", matmul_storage_type, kCodecTransformerLayers, kCodecTransformerKVHeads, kCodecPostIntermediateSize);
    for (int64_t i = 0; i < 2; ++i) {
...
struct DecodeGraph {
    DecodeGraph(
        ...
        : assets_(std::move(assets)),
          ...
          constants_(backend_, threads_, "Audio8 TTS codec decode constants") {
    ...
    runtime::AudioBuffer run(const Audio8TtsCodes & codes) {
        ...
        if (codes.codebooks != codebooks || codes.frames <= 0 ||
            ...
            oss << "Audio8 TTS codec decode code shape mismatch: expected_codebooks=" << codebooks
        ...
        if (codes.frames > frame_capacity_) {
            throw std::runtime_error("Audio8 TTS codec decode request exceeds graph capacity");
        }
...
struct EncodeGraph {
    EncodeGraph(
        ...
        : assets_(std::move(assets)),
          ...
          frame_capacity_(frames),
          constants_(backend_, threads_, "Audio8 TTS codec encode constants") {
        ggml_init_params params{graph_arena_bytes, nullptr, true};
        ctx_.reset(ggml_init(params));
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize Audio8 TTS codec encode graph context");
        }
        core::ModuleBuildContext ctx{ctx_.get(), "audio8_tts.codec.encode", backend_type_};
        constants_.begin_graph();
        ...
        trace_outputs_.push_back({"audio8_tts.codec.encoder_latent", encoded});
        ...
        gallocr_.reset(ggml_gallocr_new(ggml_backend_get_default_buffer_type(backend_)));
        if (gallocr_ == nullptr || !ggml_gallocr_alloc_graph(gallocr_.get(), graph_)) {
            throw std::runtime_error("failed to allocate Audio8 TTS codec encode graph");
        }
    }
    ...
    Audio8TtsCodes run(const runtime::AudioBuffer & audio) {
        ...
        if (padded_samples > sample_capacity_ || frames > frame_capacity_) {
            throw std::runtime_error("Audio8 TTS codec encode request exceeds graph capacity");
        }
        ...
        ggml_backend_synchronize(backend_);
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("Audio8 TTS codec encode graph compute failed");
        }
        ...
        engine::debug::trace_log_i32(
            "audio8_tts.codec.reference_codes",
...
class Audio8TtsCodecRuntime::Impl {
public:
    Impl(
        std::shared_ptr<const Audio8TtsAssets> assets,
...
Audio8TtsCodecRuntime::Audio8TtsCodecRuntime(
    std::shared_ptr<const Audio8TtsAssets> assets,
    core::BackendConfig backend,
    int threads,
    size_t graph_arena_bytes,
...
Audio8TtsCodecRuntime::~Audio8TtsCodecRuntime() = default;

Audio8TtsCodes Audio8TtsCodecRuntime::encode_reference(const runtime::AudioBuffer & audio) {
    return impl_->encode_reference(audio);
}

runtime::AudioBuffer Audio8TtsCodecRuntime::decode(const Audio8TtsCodes & codes) {
    return impl_->decode(codes);
}

void Audio8TtsCodecRuntime::release_encode_graph() {
    impl_->release_encode_graph();
}

void Audio8TtsCodecRuntime::release_runtime_graphs() {
