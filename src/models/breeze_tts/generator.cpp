...
modules::QwenDecoderActivationCastPolicy breeze_bf16_activation_policy(core::BackendType backend_type) {
    ...
    // CUDA/HIP/Vulkan implement the fused round-to-bf16 unary op; Metal does not,
    // so it falls back to an explicit cast.
...
modules::QwenCausalDecodeRuntimeConfig backbone_config(
    ...
    if (backend_type == core::BackendType::Cuda || backend_type == core::BackendType::Hip ||
        backend_type == core::BackendType::Vulkan || backend_type == core::BackendType::Metal) {
        // BF16 KV cache matches the reference implementation. Flash attention
        // only accelerates a bf16 cache with native bf16 MMA (sm_80+); on older
        // parts it is ~3x slower, so it stays F16 on CUDA/Vulkan. HIP and Metal
        // take the bf16 cache.
...
modules::QwenCausalDecodeRuntimeConfig depth_config(
    ...
    if (backend_type == core::BackendType::Cuda || backend_type == core::BackendType::Hip ||
        backend_type == core::BackendType::Vulkan || backend_type == core::BackendType::Metal) {
        // See backbone_config: HIP and Metal use a bf16 KV cache; CUDA and
        // Vulkan keep F16.
        out.decoder.static_cache_type =
            (backend_type == core::BackendType::Hip || backend_type == core::BackendType::Metal)
                ? GGML_TYPE_BF16 : GGML_TYPE_F16;
